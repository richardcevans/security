...
echo -e "${PURPLE}First run ./04_get_entra_oauth_token.sh --headless and sign in as Marvin.${NC}"
...
if [ ! -s "$AZURE_TOKEN_FILE" ]; then
  ...
  echo -e "${YELLOW}Run ./04_get_entra_oauth_token.sh --headless and sign in as ${MARVIN_UPN}.${NC}"
  exit 1
fi
