...
require_adb_entra_env() {
  ...
  if [ ! -f "${TNS_ADMIN}/tnsnames.ora" ] || [ ! -f "${TNS_ADMIN}/sqlnet.ora" ]; then
    echo "ERROR: Wallet files are missing under TNS_ADMIN=${TNS_ADMIN}. Re-run ./00_setup_adb_entra_id.sh." >&2
    exit 1
  fi

  if grep -q 'DIRECTORY="?/network/admin"' "${TNS_ADMIN}/sqlnet.ora" 2>/dev/null; then
    echo "ERROR: ${TNS_ADMIN}/sqlnet.ora still points to ?/network/admin. Re-run ./00_setup_adb_entra_id.sh." >&2
    exit 1
  fi
}
