...
ACTIONS = {
    ...
    for action in (
        ...
        _action("customer_sales", "Customer Sales", "Navigate to the Oracle Customer Sales application to view Marvin's current access.", requires=("enable_full_access",), link_step=True, short_label="Customer Sales", expect="Sign in as Marvin and select Load Customers. You should see 22 rows, including Apex Treasury, with Credit Limit and Sensitive Identifier both visible."),
        ...
        _action("customize_manager_grant", "Customize the manager grant", "Choose exactly which columns Marvin's manager role can see, then apply the change directly to Oracle. sensitive_identifier is never offered, so it remains off-limits.", requires=("create_manager_context", "enable_manager"), custom_grant_step=True, short_label="Customize grant", expect="The grant updates immediately. Reload Load Customers in Customer Sales as Marvin, manager, to see the columns you kept or removed."),
