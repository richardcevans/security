"""Publish an OCI-native metadata view of the Order History Iceberg table.
...
references. This publisher copies and rewrites the metadata graph into a
separate prefix, without copying the source Parquet files, then records the
rewritten root metadata URL for the database bootstrap.
"""
...
def rewrite_manifest_record(value: Any, bucket: str, namespace: str, region: str) -> Any:
    """Rewrite data/delete-file paths to the native URL of their source object."""
    ...
    for key, child in value.items():
        if key == "file_path" and isinstance(child, str) and child.startswith(f"s3://{bucket}/"):
            rewritten[key] = native_url(bucket, namespace, region, s3_key(child, bucket))
        else:
...
def rewrite_json(value: Any, bucket: str, namespace: str, region: str) -> Any:
    """Point copied metadata at copied metadata files and source Parquet files."""
    if isinstance(value, list):
    ...
    for key, child in value.items():
        if key == "location" and isinstance(child, str) and child == f"s3://{bucket}/{SOURCE_PREFIX}/{NAMESPACE}/{TABLE_NAME}":
            ...
            if object_name.startswith(f"{SOURCE_PREFIX}/{NAMESPACE}/{TABLE_NAME}/metadata/"):
...
def main() -> None:
    ...
    if target_info:
        raise RuntimeError(
            ...
            "destroy the Stack before redeploying or use a new bucket."
        )

    metadata_selector = pa.fs.FileSelector(
        f"{bucket}/{SOURCE_PREFIX}/{NAMESPACE}/{TABLE_NAME}/metadata",
        recursive=True,
        allow_not_found=False,
    )
    source_keys = []
    for info in filesystem.get_file_info(metadata_selector):
    ...
    json_keys = sorted(key for key in payloads if key.endswith(".metadata.json"))
    if not json_keys:
