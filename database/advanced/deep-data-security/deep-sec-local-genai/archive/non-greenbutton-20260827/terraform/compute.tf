resource "oci_core_instance" "flask" {
  ...
  metadata = {
    ...
    user_data = base64gzip(templatefile("${path.module}/templates/genai-defaults-cloud-init.yaml.tftpl", {
      ...
      order_history_access_key       = oci_identity_customer_secret_key.order_history_upload.id
      order_history_secret_key       = oci_identity_customer_secret_key.order_history_upload.key
