...
# The Marketplace stack uses the same isolated, offline-safe application
# sources as GreenButton but publishes a separately named application bundle.
...
archive="$script_dir/dist/deep-data-security-flask-app-Marketplace.zip"
rm -f "$archive" "$archive.sha256"
(cd "$stage_dir" && zip -qr "$archive" flask-app admin-app setup vibe-cli.zip)
unzip -tq "$archive"
sha256sum "$archive" > "$archive.sha256"
echo "Created dist/deep-data-security-flask-app-Marketplace.zip and its SHA-256 file."
