...
# Build the application payload independently from the MarketplaceSeparate
# Terraform stack. Re-upload this object when the application changes.
...
archive="$script_dir/dist/deep-data-security-flask-app-MarketplaceSeparate.zip"
rm -f "$archive" "$archive.sha256"
(cd "$stage_dir" && zip -qr "$archive" flask-app admin-app setup vibe-cli.zip)
unzip -tq "$archive"
sha256sum "$archive" > "$archive.sha256"
echo "Created dist/deep-data-security-flask-app-MarketplaceSeparate.zip and its SHA-256 file."
