...
# Build the application payload independently from the WilliamLiveLab
...
archive="$script_dir/dist/deep-data-security-flask-app-WilliamLiveLab.zip"
rm -f "$archive" "$archive.sha256"
(cd "$stage_dir" && zip -qr "$archive" flask-app admin-app setup vibe-cli.zip)
unzip -tq "$archive"
sha256sum "$archive" > "$archive.sha256"
echo "Created dist/deep-data-security-flask-app-WilliamLiveLab.zip and its SHA-256 file."
