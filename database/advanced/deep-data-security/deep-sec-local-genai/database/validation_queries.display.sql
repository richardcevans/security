-- What this step does. Safe to copy — no live secrets appear here.
-- Run as the attached local database end user. The Flask application uses the
-- same broad query before and after every authorization change.

-- 1. Request every customer column and sort by revenue. Oracle's active data
--    grants decide which rows and columns are actually returned.
SELECT *
  FROM APPLAB.CUSTOMERS
 ORDER BY REVENUE DESC;

-- 2. Show the authenticated end user Oracle used for the authorization decision.
SELECT ORA_END_USER_CONTEXT.USERNAME AS END_USER FROM DUAL;

-- 3. Show the active Oracle Deep Data Security data roles for this session.
SELECT ROLE_NAME AS ACTIVE_DATA_ROLE
  FROM V$END_USER_DATA_ROLE
 ORDER BY ROLE_NAME;
