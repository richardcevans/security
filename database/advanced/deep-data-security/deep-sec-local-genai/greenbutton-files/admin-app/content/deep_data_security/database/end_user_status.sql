...
with grants as (
  ...
  select g.grant_name, g.object_name, g.privilege, c.column_name
    from dba_data_grants g join obj_cols c on c.object_name = g.object_name and (g.column_name = c.column_name or g.column_name is null)
   where g.object_owner = 'APPLAB' and g.privilege in ('SELECT', 'UPDATE')
), grant_delete as (
...
select gr.grant_name,
  ...
  from grants gr left join grant_priv gp on gp.grant_name = gr.grant_name
...
select gr.grant_name,
       ...
       case when gd.grant_name is not null then 'YES' else 'NO' end as can_delete
  from grants gr join obj_cols oc on oc.object_name = gr.object_name
