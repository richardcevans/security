...
select data_role as role_name, mapped_to, enabled_by_default from dba_data_roles order by data_role;
...
select grant_name, privilege, column_name, grantee from dba_data_grants where object_owner = 'APPLAB' order by grant_name, privilege, column_name;

prompt Predicate for each grant, shown once rather than once per column.
column predicate format a70
select distinct grant_name, predicate from dba_data_grants where object_owner = 'APPLAB' order by grant_name;
...
select username, account_status, created_date from dba_end_users order by username;
