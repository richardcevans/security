...
The custom image must pre-load the verified Python 3.9 wheelhouse at
`/opt/deep-sec-offline/wheelhouse/py3.9`. On a golden VM, stage it before
creating the image with `sudo env OFFLINE_ROOT=/home/opc/aiworld-offline/flask-python
TARGET_ROOT=/opt/deep-sec-offline ./stage_greenbutton_offline_wheelhouse.sh`.
...
supplied custom image must also provide `/usr/bin/python3` with `venv` and the
...
`region` defaults to `us-ashburn-1`. The Compute image OCID is
tenancy- and region-specific: a team using a
