resource "oci_core_instance" "flask" {
  ...
  depends_on = [
    ...
    # The Stack-created bucket and object PARs must be ready before cloud-init
    # materializes the table graph downloaded from the external bundle PAR.
    oci_objectstorage_preauthrequest.order_history_bundle_object_write,
    oci_core_app_catalog_subscription.marketplace_image,
  ]

  lifecycle {
    precondition {
      condition     = var.use_marketplace_image || trimspace(var.compute_image_ocid) != ""
      error_message = "Provide a custom image OCID when use_marketplace_image is false."
    }
    precondition {
      condition     = !var.use_marketplace_image || length(data.oci_core_app_catalog_listing_resource_version.marketplace_image) == 1
      error_message = "The Marketplace listing resource version must resolve to a launchable image in the consuming tenancy."
    }
  }
  ...
  source_details {
    source_type = "image"
    source_id   = local.marketplace_image_source_id
  }
  ...
  metadata = {
    ...
    user_data = base64gzip(templatefile("${path.module}/templates/genai-defaults-cloud-init.yaml.tftpl", {
      ...
      # deliberately not called by Marketplace.
      adb_service_alias                   = ""
      ...
      order_history_bundle_archive_sha256 = ""
