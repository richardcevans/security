...
resource "random_id" "bootstrap_status_suffix" {
  ...
  # Re-key when either external component URL, cloud-init template, injected
  # SSH key, or subnet changes, all of which can cause a fresh bootstrap.
  keepers = {
    application_bundle_par_url   = var.application_bundle_par_url
    order_history_bundle_par_url = var.order_history_bundle_par_url
...
# write PARs, so the bucket cleanup must run only after those PARs are gone.
...
# Cloud-init writes the downloaded Iceberg sample files directly to this
