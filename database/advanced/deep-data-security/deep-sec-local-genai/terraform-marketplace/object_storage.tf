...
resource "random_id" "bootstrap_status_suffix" {
  ...
  # Re-key when either artifact, cloud-init template, injected SSH key, or
  # subnet changes, all of which can cause a fresh bootstrap.
  keepers = {
    application_zip = filesha256("${path.module}/artifacts/deep-data-security-flask-app-Marketplace.zip")
...
resource "oci_objectstorage_object" "marketplace_app" {
  bucket       = oci_objectstorage_bucket.wallet.name
  namespace    = data.oci_objectstorage_namespace.current.namespace
  object       = "deep-data-security-flask-app-Marketplace.zip"
  source       = "${path.module}/artifacts/deep-data-security-flask-app-Marketplace.zip"
...
resource "oci_objectstorage_preauthrequest" "marketplace_app_read" {
  bucket       = oci_objectstorage_bucket.wallet.name
  namespace    = data.oci_objectstorage_namespace.current.namespace
  name         = "${local.stack_resource_prefix}-app-read"
  access_type  = "ObjectRead"
  object_name  = oci_objectstorage_object.marketplace_app.object
