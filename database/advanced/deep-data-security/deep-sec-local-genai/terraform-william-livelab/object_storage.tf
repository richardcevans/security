...
resource "random_id" "bootstrap_status_suffix" {
  ...
  keepers = {
    application_bundle_par_url = var.application_bundle_par_url
    ...
    auth_token_id              = oci_identity_auth_token.order_history.id
  }
}
...
# Object Storage bucket creation can take time to propagate. Wait before
# uploading the bundled Iceberg artifact so subsequent PAR creation is reliable.
resource "time_sleep" "wait_for_buckets" {
  depends_on = [
    oci_objectstorage_bucket.wallet,
    oci_objectstorage_bucket.order_history,
  ]
...
resource "terraform_data" "bootstrap_verification" {
  input = {
    ...
    jupyter_url     = "http://${oci_core_instance.flask.public_ip}:8888/"
    status_read_url = "https://objectstorage.${var.ociRegionIdentifier}.oraclecloud.com${oci_objectstorage_preauthrequest.bootstrap_status_read.access_uri}"
  }
    ...
    command     = <<-EOT
      ...
      while (( $(date +%s) < deadline )); do
        ...
        if [[ "$http_code" == '200' ]]; then
          cat "$status_file"
          current_run=$(sed -n 's/.* run=\([^ ]*\).*/\1/p' "$status_file")
          if [[ -z "$baseline_run" && -n "$current_run" ]]; then
            baseline_run="$current_run"
            if grep -Eq '^(COMPLETE|FAILED) ' "$status_file"; then
              baseline_terminal='true'
            fi
          fi
          if grep -q '^COMPLETE ' "$status_file"; then
            if [[ "$baseline_terminal" == 'true' && "$current_run" == "$baseline_run" ]]; then
              echo 'Ignoring a terminal marker from the prior bootstrap run.'
            else
