...
if [ ! -f "$SQLNET_FILE" ]; then
  echo -e "${RED}ERROR: ${SQLNET_FILE} was not found. Run DOWNLOAD_WALLET=1 ./00_configure_lab_env.sh or install the ADB wallet manually.${NC}"
  exit 1
fi
...
echo -e "${GREEN}============================================================================${NC}"
echo
echo "Ready: run ./05_verify_as_manager.sh or ./06_verify_as_employee.sh"
echo
