#!/bin/bash
# Verify DeepSec MCP OCI IAM data grants with a Marvin OAuth2 token.
...
echo -e "${CYAN}TOKEN_LOCATION=${OCI_TOKEN_DIR:-$HOME/.oci/deep-sec-mcp}${NC}"
require_sqlplus
...
echo
echo -e "${GREEN}Task 5 completed. Next: clear the token, get an Emma token, then run ./06_verify_as_employee.sh${NC}"
echo
