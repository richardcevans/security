...
INSTANCE_FILE="${SCRIPT_DIR}/.deep-sec-mcp.instance"
WORK_DIR="${SCRIPT_DIR}/.deep-sec-mcp-setup"
...
usage() {
  cat <<'EOF'
Usage:
  ./setup_adbs_oci_iam.sh [compartment-name|compartment-ocid|root]
...
require_non_production_acknowledgement() {
  ...
  echo -e "${CYAN}IAM groups          = ${OCI_IAM_EMPLOYEE_GROUP_DISPLAY_NAME}, ${OCI_IAM_MANAGER_GROUP_DISPLAY_NAME}${NC}"
  echo -e "${CYAN}Data role mappings  = ${DATA_ROLE_MAPPING_TYPE}:${OCI_IAM_EMPLOYEE_GROUP}, ${OCI_IAM_MANAGER_GROUP}${NC}"
...
DEEPSEC_MCP_LAB_INSTANCE_ID=$(make_lab_instance_id "dbsec-lab-machine" "$INSTANCE_FILE" "DEEPSEC_MCP_LAB_INSTANCE_ID")
export DEEPSEC_MCP_LAB_INSTANCE_ID
...
if [ -z "${OCI_DB_APP_NAME:-}" ] || [ "$OCI_DB_APP_NAME" = "$legacy_oci_db_app_name" ] || [ "$OCI_DB_APP_NAME" = "DeepSec MCP DB Resource" ]; then
  OCI_DB_APP_NAME="${DB_NAME} DeepSec MCP DB Resource - ${DEEPSEC_MCP_LAB_INSTANCE_ID}"
fi
if [ -z "${OCI_CLIENT_APP_NAME:-}" ] || [ "$OCI_CLIENT_APP_NAME" = "$legacy_oci_client_app_name" ] || [ "$OCI_CLIENT_APP_NAME" = "DeepSec MCP Public Client" ]; then
  OCI_CLIENT_APP_NAME="${DB_NAME} DeepSec MCP Public Client - ${DEEPSEC_MCP_LAB_INSTANCE_ID}"
fi
...
if [ -z "${OCI_DB_AUDIENCE:-}" ] || [ "$OCI_DB_AUDIENCE" = "OracleDB" ] || [ "$OCI_DB_AUDIENCE" = "${DB_NAME}OracleDB" ]; then
  OCI_DB_AUDIENCE="${DB_NAME}OracleDB${DEEPSEC_MCP_LAB_INSTANCE_SHORT}"
fi
if [ -z "${OCI_DB_SCOPE_VALUE:-}" ] || [ "$OCI_DB_SCOPE_VALUE" = "DB_ACCESS_SCOPE" ] || [ "$OCI_DB_SCOPE_VALUE" = "${DB_NAME}_DB_ACCESS_SCOPE" ]; then
  OCI_DB_SCOPE_VALUE="${DB_NAME}_DB_ACCESS_SCOPE_${DEEPSEC_MCP_LAB_INSTANCE_SHORT}"
fi
...
create_or_reuse_db_resource_app() {
  ...
  if [ -z "$app_id" ]; then
    ...
    if app_id=$(domain_cmd app create \
      ...
      --display-name "$OCI_DB_APP_NAME" \
      --description "Database resource app for the DeepSec MCP Deep Data Security lab" \
      --active true \
...
create_or_reuse_public_client_app() {
  ...
  if [ -n "$app_id" ]; then
    ...
    app_id=$(domain_cmd app create \
      ...
      --based-on-template '{"value":"CustomWebAppTemplateId","wellKnownId":"CustomWebAppTemplateId"}' \
      --display-name "$OCI_CLIENT_APP_NAME" \
      --description "Public interactive OAuth client for the DeepSec MCP Deep Data Security lab" \
      --active true \
      --is-o-auth-client true \
      --client-type public \
      --allowed-grants '["authorization_code"]' \
...
if [ -z "${ROOT_COMP_ID:-}" ]; then
  ...
  if [ "$oci_compartment_lc" = "root" ]; then
    ...
    if [ -z "$ROOT_COMP_ID" ] || [ "$ROOT_COMP_ID" = "null" ]; then
      ...
      echo "  ./setup_adbs_oci_iam.sh root"
      echo "  ./setup_adbs_oci_iam.sh <compartment-name>"
...
MCP_IDENTITY_DOMAIN_OCID="${MCP_IDENTITY_DOMAIN_OCID:-$(oci iam domain list \
  ...
  --raw-output 2>/dev/null || true)}"
export MCP_IDENTITY_DOMAIN_OCID
require_non_production_acknowledgement
...
echo -e "${CYAN}  LAB_INSTANCE_ID = ${DEEPSEC_MCP_LAB_INSTANCE_ID}${NC}"
...
echo -e "${CYAN}  IAM groups      = ${OCI_IAM_EMPLOYEE_GROUP_DISPLAY_NAME}, ${OCI_IAM_MANAGER_GROUP_DISPLAY_NAME}${NC}"
echo -e "${CYAN}  Data role map   = ${DATA_ROLE_MAPPING_TYPE}:${OCI_IAM_EMPLOYEE_GROUP}, ${OCI_IAM_MANAGER_GROUP}${NC}"
...
if [ -z "${DATABASE_TOOLS_CONNECTION_STRING:-}" ]; then
  DATABASE_TOOLS_CONNECTION_STRING=$(extract_tns_descriptor || true)
  export DATABASE_TOOLS_CONNECTION_STRING
fi

echo
echo -e "${YELLOW}Step 5: Creating or reusing OCI IAM domain groups and demo users...${NC}"
EMPLOYEES_OCID=$(create_or_reuse_domain_group "$OCI_IAM_EMPLOYEE_GROUP_DISPLAY_NAME")
MANAGERS_OCID=$(create_or_reuse_domain_group "$OCI_IAM_MANAGER_GROUP_DISPLAY_NAME")
...
if [ "$CREATE_DEMO_USERS" = "1" ]; then
  ...
  add_domain_user_to_group "$MARVIN_ID" "$EMPLOYEES_OCID" "$MARVIN_USERNAME" "$OCI_IAM_EMPLOYEE_GROUP_DISPLAY_NAME"
  add_domain_user_to_group "$MARVIN_ID" "$MANAGERS_OCID" "$MARVIN_USERNAME" "$OCI_IAM_MANAGER_GROUP_DISPLAY_NAME"
  add_domain_user_to_group "$EMMA_ID" "$EMPLOYEES_OCID" "$EMMA_USERNAME" "$OCI_IAM_EMPLOYEE_GROUP_DISPLAY_NAME"
else
  echo -e "${YELLOW}  CREATE_DEMO_USERS=0, so Marvin and Emma users were not created.${NC}"
  echo -e "${YELLOW}  Ensure ${MARVIN_USERNAME} has ${OCI_IAM_EMPLOYEE_GROUP_DISPLAY_NAME}, ${OCI_IAM_MANAGER_GROUP_DISPLAY_NAME}.${NC}"
  echo -e "${YELLOW}  Ensure ${EMMA_USERNAME} has ${OCI_IAM_EMPLOYEE_GROUP_DISPLAY_NAME}.${NC}"
fi

NAMESPACE="${NAMESPACE:-$(oci os ns get --raw-output --query data "${oci_global_args[@]}" 2>/dev/null || true)}"
MCP_COMPARTMENT_OCID="${MCP_COMPARTMENT_OCID:-$ROOT_COMP_ID}"
MCP_COMPARTMENT_NAME="${MCP_COMPARTMENT_NAME:-$OCI_COMPARTMENT}"
MCP_IDENTITY_DOMAIN_NAME="${MCP_IDENTITY_DOMAIN_NAME:-$OCI_DOMAIN_NAME}"
MCP_BUCKET_NAME="${MCP_BUCKET_NAME:-deep-sec-mcp-${DEEPSEC_MCP_LAB_INSTANCE_SHORT}-mcp}"
MCP_SERVER_NAME="${MCP_SERVER_NAME:-deep-sec-mcp-${DEEPSEC_MCP_LAB_INSTANCE_SHORT}}"
MCP_CREATE_BUILT_IN_SQL_TOOLSET="${MCP_CREATE_BUILT_IN_SQL_TOOLSET:-1}"
MCP_BUILT_IN_SQL_TOOLSET_NAME="${MCP_BUILT_IN_SQL_TOOLSET_NAME:-deep-sec-mcp-${DEEPSEC_MCP_LAB_INSTANCE_SHORT}-built-in-sql-tools}"
MCP_BUILT_IN_SQL_TOOLSET_VERSION="${MCP_BUILT_IN_SQL_TOOLSET_VERSION:-1}"
DATABASE_TOOLS_CONNECTION_NAME="${DATABASE_TOOLS_CONNECTION_NAME:-deep-sec-mcp-${DEEPSEC_MCP_LAB_INSTANCE_SHORT}-connection}"
...
export OCI_DOMAIN_NAME='${OCI_DOMAIN_NAME}'
...
export DEEPSEC_MCP_LAB_INSTANCE_ID='${DEEPSEC_MCP_LAB_INSTANCE_ID}'
export OCI_DB_APP_NAME='${OCI_DB_APP_NAME}'
export OCI_CLIENT_APP_NAME='${OCI_CLIENT_APP_NAME}'
export DATA_ROLE_MAPPING_TYPE='${DATA_ROLE_MAPPING_TYPE}'
export OCI_IAM_EMPLOYEE_GROUP_DISPLAY_NAME='${OCI_IAM_EMPLOYEE_GROUP_DISPLAY_NAME}'
export OCI_IAM_MANAGER_GROUP_DISPLAY_NAME='${OCI_IAM_MANAGER_GROUP_DISPLAY_NAME}'
...
export MCP_RUNTIME_IDENTITY='${MCP_RUNTIME_IDENTITY}'
export MCP_CREATE_BUILT_IN_SQL_TOOLSET='${MCP_CREATE_BUILT_IN_SQL_TOOLSET}'
export MCP_BUILT_IN_SQL_TOOLSET_NAME='${MCP_BUILT_IN_SQL_TOOLSET_NAME}'
export MCP_BUILT_IN_SQL_TOOLSET_VERSION='${MCP_BUILT_IN_SQL_TOOLSET_VERSION}'
export DATABASE_TOOLS_CONNECTION_NAME='${DATABASE_TOOLS_CONNECTION_NAME}'
export DATABASE_TOOLS_CONNECTION_STRING='${DATABASE_TOOLS_CONNECTION_STRING}'
export DATABASE_TOOLS_CONNECTION_AUTHENTICATION_TYPE='${DATABASE_TOOLS_CONNECTION_AUTHENTICATION_TYPE}'
export DATABASE_TOOLS_RUNTIME_IDENTITY='${DATABASE_TOOLS_RUNTIME_IDENTITY}'
EOF
chmod 600 "$ENV_FILE"
