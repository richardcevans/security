...
variable "base_db_system_ocid" {
  description = "Existing Oracle Base Database System OCID used as the related Database Tools resource."
  type        = string
  default     = "ocid1.dbsystem.oc1.iad.replace"
}

variable "base_database_ocid" {
  description = "Existing database OCID inside the Base Database System."
  type        = string
  default     = "ocid1.database.oc1.iad.replace"
}

variable "base_db_connection_string" {
  description = "Oracle Net connect string for the target database service, for example host:1521/service_name or a full DESCRIPTION string."
  type        = string
...
variable "create_database_tools_vault_secret" {
  description = "Create one OCI Vault, key, and password secret for Database Tools connections. Uses database_tools_connection_password as the secret value."
  type        = bool
  default     = false
}
...
variable "create_database_tools_connections" {
  description = "Create Database Tools connections for each participant from the existing Base Database System connection string."
  type        = bool
  default     = false
}
...
variable "database_tools_connection_password" {
  description = "Password for Database Tools connection secret creation."
  type        = string
  sensitive   = true
  default     = null
}
...
variable "database_tools_related_resource_entity_type" {
  description = "Database Tools related resource entity type. DATABASE is preferred when a database OCID is known; use DB_SYSTEM only if required by the service."
  type        = string
  default     = "DATABASE"
}

variable "database_tools_related_resource_ocid" {
  description = "Optional override for the Database Tools related resource OCID. Defaults to base_database_ocid, then base_db_system_ocid."
  type        = string
  default     = null
}
