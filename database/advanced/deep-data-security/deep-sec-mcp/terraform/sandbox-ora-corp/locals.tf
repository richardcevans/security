locals {
  generated_participants = {
    ...
    format("%s%05d", var.participant_prefix, var.participant_start_number + i) => {
      db_name      = format("DMC%05d", var.participant_start_number + i)
      display_name = format("%s-%s%05d-adb", var.name_prefix, var.participant_prefix, var.participant_start_number + i)
    }
  }
