# DeepSec MCP LiveLabs Sandbox Terraform Starter

Estimated Time: 5 minutes

### Objectives

Use this page to review the Terraform package inputs, provisioning model, and validation steps.
...
It is designed for participant-level isolation at scale. Each participant gets a dedicated compartment and dedicated mutable resources so concurrent users do not collide.
...
1. **LiveLabs/bootstrap layer**
   ...
   - Creates participant IAM or identity-domain users/groups.
...
- per-participant least-privilege policies referencing pre-created participant groups
...
- identity-domain users such as `U12345a` and `U12345b`
...
By default, the stack generates participant keys such as `U12345` and identity-plan outputs such as `U12345a` and `U12345b`. Use the optional `participants` map only when LiveLabs needs explicit participant names or database names.
...
The variable type is `map(object({ db_name = string, display_name = optional(string), restricted_user = optional(string), privileged_user = optional(string) }))`. An explicit override must use Terraform map syntax, not JSON strings.
...
- Use pre-provisioned identity-domain artifacts. Participants should not create identity-domain apps, users, groups, policies, or compartments.
