...
output "participant_identity_plan" {
  description = "Suggested per-participant identity-domain users. Create these through LiveLabs identity automation."
  value = {
    for key, value in local.effective_participants :
    key => {
      group_name      = local.participant_group_names[key]
      restricted_user = coalesce(value.restricted_user, "${key}a")
      privileged_user = coalesce(value.privileged_user, "${key}b")
    }
  }
}
