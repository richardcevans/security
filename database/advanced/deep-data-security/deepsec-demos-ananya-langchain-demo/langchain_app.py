...
from db_connection import create_connection_pool, get_end_user_identity
...
SHOW_INTERNAL_TOOL_TRACE = False
...
class PendingUpdate:
    """One proposed database mutation awaiting explicit CLI confirmation."""
...
def main():
    ...
    cfg = AppConfig()
    end_user_identity = get_end_user_identity(agent_mode=agent_mode)
    ...
    try:
        ...
        model_id = cfg.model_id or MODEL_IDS.get(cfg.model_choice, "")
        if not model_id:
            raise RuntimeError(
                "Set MODEL_ID in .env, or set MODEL_CHOICE to a key in MODEL_IDS."
            )
