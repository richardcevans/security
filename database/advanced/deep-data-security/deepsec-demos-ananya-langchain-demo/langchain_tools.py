...
def _qualify_unqualified_mutation_table(query: str) -> str:
    """Qualify supported unqualified mutation targets with the HR owner."""
...
def execute_confirmed_mutation(connection_factory, query: str) -> str:
    ...
    end-user context, allowed columns, and row predicates.
    """
...
def build_system_prompt(user: str, agent_mode: str = "hr") -> str:
    ...
    - Confirmed changes are limited to UPDATE on HR.EMPLOYEES or
      HR.PERFORMANCE_NOTES, INSERT into HR.PERFORMANCE_NOTES, and DELETE from
      HR.PERFORMANCE_NOTES. UPDATE and DELETE statements must include a WHERE
      clause.
    - For another employee's record, first identify the employee_id. For
      performance-note changes, identify the note_id first.
    - For a self-service employee update using "my", "me", or "I", the
      proposed SQL must use UPPER(email) = UPPER(ORA_END_USER_CONTEXT.username)
      in its WHERE clause. Never substitute a literal email, a remembered
      name, or an invented employee_id for ORA_END_USER_CONTEXT.username.
...
def create_tools(connection_factory, agent_mode: str = "hr"):
    ...
    def get_employee(employee_identifier: str) -> str:
        """
        Return a single employee by employee ID or email address.
        ...
        try:
            with connection_factory() as conn:
                with conn.cursor() as cursor:
                    ...
                    # Otherwise treat the identifier as an email address.
                    else:
                        cursor.execute(
                            """
                            SELECT employee_id,
    ...
    def search_employees(query: str, limit: int = 10) -> str:
        """
        Search employees by name, email address, or job ID.
                    ...
                    cursor.execute(
                        ...
                        FROM (
                            ...
                            WHERE UPPER(first_name || ' ' || last_name) LIKE UPPER(:pattern)
                               ...
                               OR UPPER(job_id) LIKE UPPER(:pattern)
    ...
    @tool
    def get_job_pay_range(job_id: str | None = None) -> str:
        """
        Return salary statistics for one job, or for all jobs if no job_id is given.
            ...
            with connection_factory() as conn:
                with conn.cursor() as cursor:
                    ...
                    if not rows:
                        if job_id is None:
                            return "NO_DATA_FOUND"
                        return f"NO_DATA_FOUND for job_id {job_id}"
        ...
        if re.match(r"^UPDATE\s+(?:HR\.)?EMPLOYEES\b", clean_sql, re.IGNORECASE):
            if "ORA_END_USER_CONTEXT.USERNAME" not in clean_sql.upper():
                return (
                    ...
                    "UPPER(email) = UPPER(ORA_END_USER_CONTEXT.username) "
                    "in the WHERE clause. Do not use a literal email or "
                    "employee_id."
                )
