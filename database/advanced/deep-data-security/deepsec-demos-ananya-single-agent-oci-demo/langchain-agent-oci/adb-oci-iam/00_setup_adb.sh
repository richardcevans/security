...
if [ -z "${OCI_CLIENT_APP_NAME:-}" ] || [ "$OCI_CLIENT_APP_NAME" = "$legacy_oci_client_app_name" ] || [ "$OCI_CLIENT_APP_NAME" = "ADB OCI IAM Public Client" ]; then
  OCI_CLIENT_APP_NAME="${DB_NAME} LangChain Confidential Login Client - ${ADB_OCI_IAM_LAB_INSTANCE_ID}"
fi
if [ -z "${OCI_MIDTIER_APP_NAME:-}" ] || [ "$OCI_MIDTIER_APP_NAME" = "ADB OCI IAM Midtier App" ]; then
  OCI_MIDTIER_APP_NAME="${DB_NAME} LangChain HR Midtier App - ${ADB_OCI_IAM_LAB_INSTANCE_ID}"
fi
...
if [ -z "${OCI_MIDTIER_AUDIENCE:-}" ]; then
  OCI_MIDTIER_AUDIENCE="${DB_NAME}LangChainHR${ADB_OCI_IAM_LAB_INSTANCE_SHORT}"
fi
...
create_or_reuse_midtier_app() {
  ...
  if [ -n "$app_id" ]; then
    ...
    app_id=$(domain_cmd app create \
      ...
      --based-on-template '{"value":"CustomWebAppTemplateId","wellKnownId":"CustomWebAppTemplateId"}' \
      --display-name "$OCI_MIDTIER_APP_NAME" \
      --description "LangChain HR midtier app for ADB Deep Data Security" \
      --active true \
      ...
      --client-secret "$generated_secret" \
      --audience "$OCI_MIDTIER_AUDIENCE" \
      --scopes "[{\"value\":\"${OCI_MIDTIER_SCOPE_VALUE}\",\"displayName\":\"Agent Access\",\"description\":\"Access the LangChain HR agent\",\"requiresConsent\":false}]" \
      --allowed-grants '["client_credentials"]' \
...
create_or_reuse_login_client_app() {
  ...
  if [ -n "$app_id" ]; then
    ...
    app_id=$(domain_cmd app create \
      ...
      --based-on-template '{"value":"CustomWebAppTemplateId","wellKnownId":"CustomWebAppTemplateId"}' \
      --display-name "$OCI_CLIENT_APP_NAME" \
      --description "Confidential interactive login client for the LangChain HR agent" \
      --active true \
      --is-o-auth-client true \
...
OCI_AUTH_TYPE=${OCI_CLI_AUTH:-API_KEY}
...
# Oracle Database connection used by the LangChain app.
DB_USER=${APP_DB_USER}
...
# OCI IAM / Deep Data Security application values.
HR_OCI_DOMAIN_URL=${OCI_DOMAIN_URL}
HR_DB_APP_ID=${OCI_DB_APP_ID}
HR_DB_CLIENT_ID=${OCI_MIDTIER_CLIENT_ID}
HR_DB_CLIENT_SECRET=${OCI_MIDTIER_CLIENT_SECRET}
HR_DB_SCOPE=${OCI_DB_SCOPE}
HR_MIDTIER_APP_ID=${OCI_MIDTIER_APP_ID}
HR_MIDTIER_CLIENT_ID=${OCI_MIDTIER_CLIENT_ID}
HR_MIDTIER_CLIENT_SECRET=${OCI_MIDTIER_CLIENT_SECRET}
HR_MIDTIER_SCOPE=${OCI_MIDTIER_SCOPE}
HR_LOGIN_APP_ID=${OCI_CLIENT_APP_ID}
HR_LOGIN_CLIENT_ID=${OCI_CLIENT_ID}
HR_LOGIN_CLIENT_SECRET=${OCI_CLIENT_SECRET}
HR_APP_CLIENT_ID=${OCI_CLIENT_ID}
HR_APP_CLIENT_SECRET=${OCI_CLIENT_SECRET}
HR_APP_SCOPE=${OCI_SCOPE}
HR_REDIRECT_URI=${OCI_REDIRECT_URI}
EOF
