...
Required environment:
  ...
  OCI_CLIENT_ID        OAuth client id for the confidential login client app
  OCI_CLIENT_SECRET    OAuth client secret for the confidential login client app
  OCI_SCOPE            HR midtier agent-access scope granted to the login client

Optional environment:
  OCI_REDIRECT_URI     Redirect URI registered on the login client app.
...
set_sqlnet_value() {
  ...
  # Some wallet sqlnet.ora files do not end with a newline. Without one, an
  # appended TOKEN_AUTH line can be joined to SSL_SERVER_DN_MATCH and silently
  # ignored by SQL*Plus (for example, SSL_SERVER_DN_MATCH=yesTOKEN_AUTH=OAUTH).
  # Normalize both that previously malformed case and the final newline before
  # editing OAuth settings.
  python3 - "$SQLNET_FILE" <<'PY'
import re
import sys
...
content = re.sub(
    r"(?im)(SSL_SERVER_DN_MATCH\s*=\s*(?:yes|true))(?=TOKEN_(?:AUTH|LOCATION)\s*=)",
    r"\1\n",
    content,
)
...
def extract_authorization_code(pasted):
    ...
    if "://" not in pasted and "code=" not in pasted:
        error_panel("PASTE THE FULL CALLBACK URL", [
            "The pasted value is not a callback URL.",
            "Copy the complete http://localhost:8888/callback?code=...&state=...",
            "URL from the browser address bar, then rerun this script.",
        ])
        sys.exit(1)
    ...
    if "error" in params:
        ...
        error_panel("OCI IAM DID NOT AUTHORIZE THE LOGIN", [
            f"OCI IAM returned {error}: {description or '(no description)'}",
            "Review the browser login error, then rerun this script with a new LOGIN URL.",
        ])
        sys.exit(1)

    returned_state = params.get("state", [""])[0]
    if returned_state and returned_state != state:
        error_panel("CALLBACK URL BELONGS TO A DIFFERENT LOGIN REQUEST", [
            "The pasted callback URL state does not match the LOGIN URL printed",
            "by this run. It was probably copied from an earlier browser session",
            "or a previous execution of this script.",
            "",
            "Recovery:",
            "1. Rerun ./04_get_iam_oauth_token.sh --headless.",
            "2. Open only the newly printed LOGIN URL in a private browser window.",
            "3. After sign-in, paste the callback URL generated by that LOGIN URL.",
        ])
        sys.exit(1)

    code = params.get("code", [""])[0]
    if not code:
        error_panel("CALLBACK URL HAS NO AUTHORIZATION CODE", [
            "The pasted URL does not contain a code= parameter.",
            "Rerun this script and paste the complete callback URL from the browser address bar.",
        ])
        sys.exit(1)
...
if "error" in result:
    error_panel("OCI IAM CALLBACK FAILED", [
        str(result["error"]),
        "Rerun this script and complete the browser login using the new LOGIN URL.",
    ])
    sys.exit(1)
...
if not code:
    error_panel("AUTHORIZATION CODE NOT RECEIVED", [
        "Rerun this script and paste the complete callback URL from the browser address bar.",
    ])
    sys.exit(1)
...
if token_response.get("_http_error"):
    ...
    try:
        oauth_error = json.loads(detail)
    except Exception:
        oauth_error = {}
    if oauth_error.get("error") == "invalid_grant":
        error_panel("AUTHORIZATION CODE IS INVALID OR EXPIRED", [
            "The authorization code can be used only once and must come from this run's LOGIN URL.",
            "Do not paste a code by itself; paste the complete callback URL.",
            "",
            "Recovery: rerun ./04_get_iam_oauth_token.sh --headless and complete a new login.",
        ])
    else:
        error_panel("OCI IAM TOKEN EXCHANGE FAILED", [
            f"HTTP {token_response['_http_error']}",
            detail or "OCI IAM did not return error details.",
            "Rerun this script with a new LOGIN URL after correcting the reported OAuth configuration issue.",
        ])
    sys.exit(1)

access_token = token_response.get("access_token")
if not access_token:
    error_panel("OCI IAM TOKEN RESPONSE IS INCOMPLETE", [
        "OCI IAM did not return an access_token.",
