...
delete_domain_apps_matching() {
  ...
  local app_list app_id app_name
  local app_count=0

  if [ -z "${OCI_DOMAIN_URL:-}" ]; then
    echo -e "${YELLOW}Skipping applications matching ${name_fragment}; OCI_DOMAIN_URL is not set.${NC}"
    record_not_completed "OCI IAM apps matching ${name_fragment} — OCI_DOMAIN_URL is not set"
    return 0
  fi
...
for resource in resources:
    name = resource.get("displayName") or resource.get("display_name") or ""
    resource_id = resource.get("id") or ""
    if resource_id and needle in name:
        print("{}|{}".format(resource_id, name))
' 2>/dev/null); then
    echo -e "${RED}ERROR: Could not list OCI IAM applications matching ${name_fragment}.${NC}"
    echo "  Next: Verify OCI Identity Domain permissions, then retry."
...
if [ "$DELETE_ADB" = true ]; then
  if [ "${ADB_CREATE_MODE:-create}" = "existing" ]; then
    echo -e "${YELLOW}Skipping ADB deletion because ADB_CREATE_MODE=existing.${NC}"
    record_not_completed "ADB ${DB_NAME} — external database was not created by this lab"
  elif [ "${ADB_OCID:-}" = "external" ]; then
    echo -e "${YELLOW}Skipping ADB deletion because ADB_OCID=external.${NC}"
    record_not_completed "ADB ${DB_NAME} — external database was not created by this lab"
  elif confirm "This deletes the Autonomous AI Database ${DB_NAME}."; then
...
if [ "$DELETE_IAM_APPS" = true ]; then
  if confirm "This deletes the lab OCI IAM OAuth applications."; then
    echo
    echo -e "${YELLOW}Deleting lab OCI IAM OAuth applications...${NC}"
    if [ -n "${DB_NAME:-}" ]; then
      # Client apps reference resource-app scopes, so remove them first.
      delete_domain_apps_matching "${DB_NAME} LangChain Confidential Login Client"
      delete_domain_apps_matching "${DB_NAME} LangChain HR Midtier App"
      delete_domain_apps_matching "${DB_NAME} ADB OCI IAM Public Client"
      delete_domain_apps_matching "${DB_NAME} ADB OCI IAM DB Resource"
    else
      delete_domain_app "${OCI_CLIENT_APP_ID:-}" "${OCI_CLIENT_APP_NAME:-LangChain Confidential Login Client}"
      delete_domain_app "${OCI_MIDTIER_APP_ID:-}" "${OCI_MIDTIER_APP_NAME:-LangChain HR Midtier App}"
...
if [ "$DELETE_ALL_LAB_APPS" = true ]; then
  if confirm "This deletes all ADB OCI IAM lab Integrated Applications in the configured Identity Domain."; then
    echo
    echo -e "${YELLOW}Deleting all ADB OCI IAM lab Integrated Applications...${NC}"
    # This order removes allowedScopes references before the resource apps/scopes.
    delete_domain_apps_matching "LangChain Confidential Login Client"
    delete_domain_apps_matching "LangChain HR Midtier App"
    delete_domain_apps_matching "ADB OCI IAM Public Client"
    delete_domain_apps_matching "ADB OCI IAM DB Resource"
  else
    echo -e "${YELLOW}Skipped all lab OCI IAM application deletion.${NC}"
    record_not_completed "All lab OCI IAM applications — deletion was not confirmed"
  fi
fi
