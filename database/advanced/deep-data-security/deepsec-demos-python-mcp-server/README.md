...
<!-- Oracle SQL MCP: an OAuth-protected, read-only Oracle Database MCP pattern. -->
...
Copy the template, then populate it with environment-specific values:

```bash
cp .env.example .env
```

Never commit `.env`, wallets, OCI configuration files, private keys, tokens,
passwords, or client secrets.

The required settings are grouped below. See [.env.example](.env.example) for
the complete sanitized template.
