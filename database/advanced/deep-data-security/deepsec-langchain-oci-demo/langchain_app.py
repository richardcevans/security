...
# conversation state and confirmation-required updates, and runs the
...
def _parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="Oracle HR demo agent")
    parser.add_argument(
        "--agent",
        choices=["hr", "compensation"],
        default="hr",
        help="Select which agent to launch",
    )
...
class PendingUpdate:
    """One proposed database mutation awaiting an explicit CLI confirmation."""
...
def _find_pending_update(messages: list[Any]) -> Optional[PendingUpdate]:
    """Extract the first proposal accepted by the proposal tool this turn."""
    tool_results = {
        getattr(msg, "tool_call_id", ""): _message_content_to_text(
            getattr(msg, "content", "")
        )
        for msg in messages
    ...
    for msg in messages:
        ...
        for call in getattr(msg, "tool_calls", None) or []:
            ...
            if name == "propose_update":
                if tool_results.get(call.get("id")) != "CONFIRMATION_REQUIRED":
                    continue
                sql = _clean_sql(str(args.get("sql", "")))
                description = str(args.get("description", "")).strip()
                if sql and description:
                    return PendingUpdate(sql=sql, description=description)
    return None


def _build_confirmation_message(pending_update: PendingUpdate) -> str:
    """Return a user-facing review message without exposing raw SQL."""
    return (
        "Confirmation required\n\n"
        f"Proposed change: {pending_update.description}\n\n"
        "Reply exactly yes to apply this change, or no to cancel it."
    )
...
def _maybe_update_state_from_user_text(
    ...
    if "employee id" in text or "employee_id" in text:
        ...
        "hire date" in text
        or "hiring date" in text
        or "hire_date" in text
    ):
        state.sort_key = "hire_date"
...
def _print_banner(user: str, agent_mode: str) -> None:
    print()

    if agent_mode == "compensation":
        print("Oracle HR Compensation Analytics Agent")
    else:
...
def _get_end_user_identity(agent_mode: str) -> str:
    """Authenticate once and return the OCI IAM token for this agent session."""
    prefix = "COMP" if agent_mode == "compensation" else "HR"
    auth_values = {
        "OCI_DOMAIN_URL": _env(f"{prefix}_OCI_DOMAIN_URL", _env("OCI_DOMAIN_URL")),
        "APP_CLIENT_ID": _env(f"{prefix}_APP_CLIENT_ID"),
        "APP_CLIENT_SECRET": _env(f"{prefix}_APP_CLIENT_SECRET"),
        "APP_SCOPE": _env(f"{prefix}_APP_SCOPE"),
        "REDIRECT_URI": _env(
    ...
    if missing:
        raise RuntimeError(
            "Missing OCI IAM login configuration for {} mode: {}".format(
    ...
    return get_access_token(
        ...
        client_secret=auth_values["APP_CLIENT_SECRET"],
        scope=auth_values["APP_SCOPE"],
...
def main():
    ...
    cfg = AppConfig()
    missing_genai_settings = [
        name
        for name, value in {
            "OCI_CONFIG_FILE": cfg.oci_config_file,
            "COMPARTMENT_ID": cfg.COMPARTMENT_ID,
            "MODEL_ID": cfg.model_id,
            "OCI_GENAI_ENDPOINT": cfg.oci_genai_endpoint,
        }.items()
        if not value
    ]
    if missing_genai_settings:
        raise RuntimeError(
            "Missing OCI Generative AI configuration: "
            + ", ".join(missing_genai_settings)
        )

    end_user_identity = _get_end_user_identity(agent_mode)
    pool = create_connection_pool(agent_mode=agent_mode)

    def connection_factory():
        # The plug-in reads this identity as it acquires each pooled connection.
    ...
    try:
        ...
        system_prompt = build_system_prompt(user, agent_mode=agent_mode)
        tools = create_tools(connection_factory, agent_mode=agent_mode)

        llm = ChatOCIGenAI(
            model_id=cfg.model_id,
            compartment_id=cfg.COMPARTMENT_ID,
            service_endpoint=cfg.oci_genai_endpoint,
            auth_type="API_KEY",
        ...
        while True:
            ...
            if pending_update is not None:
                ...
                if confirmation == "yes":
                    ...
                    result = execute_confirmed_mutation(
                        connection_factory, pending_update.sql
                    )
                    logger.info(
                        "Confirmed database change | description=%s | result=%s",
                ...
                if confirmation == "no":
                    _print_answer("The proposed change was canceled. No data was modified.")
                    history.extend(
                        [
                            HumanMessage(content="no"),
                            AIMessage(content="The proposed change was canceled."),
                        ]
                    )
                    pending_update = None
                    continue
                ...
                _print_answer(
                    "A change is awaiting confirmation. Reply exactly yes to apply it "
                    "or no to cancel it."
                )
                continue
            ...
            proposed_update = _find_pending_update(new_messages)
            if proposed_update is not None:
                pending_update = proposed_update
                confirmation_message = _build_confirmation_message(pending_update)
                logger.info(
                    "Database change proposed | description=%s",
                    pending_update.description,
                )
                _print_answer(confirmation_message)
                history.extend(
                    [
                ...
                state.last_answer = confirmation_message
