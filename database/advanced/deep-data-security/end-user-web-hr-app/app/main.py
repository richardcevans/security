...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path == "/api/salary-summary":
            ...
            if not user:
                return
            self._call_database(lambda: DATABASE.visible_summary(user))
            return
    ...
    def do_POST(self):
        ...
        if path == "/api/login":
            try:
                ...
                return
            self._set_session_json(session_id)
            return
    ...
    def _merge_debug_payload(self, payload, user):
        payload["database_auth"] = DATABASE.debug_tokens_for_user(user)
    ...
    def _set_session_json(self, session_id):
