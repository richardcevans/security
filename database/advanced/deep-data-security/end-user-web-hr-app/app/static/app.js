...
async function refreshConfig() {
  ...
  modeBanner.innerHTML = mode === "oracledb"
    ? "<strong>Oracle mode:</strong> requests authenticate directly as the selected database end user."
...
async function refreshUser() {
  ...
  if (!user) {
    ...
    userBox.innerHTML = "<strong>Not signed in</strong><br />Enter Emma or Marvin";
    return null;
  }
...
function showAuthenticationContextDemo(user) {
  raw.textContent = JSON.stringify({
    ...
    enforcement: "Oracle activates the user's data roles at login and enforces the HR data grants for every query and update."
  }, null, 2);
}
  ...
  summary.innerHTML = `
    ...
    <div>
      ...
      <dd>${escapeHtml((payload.data_roles || []).join(", "))}</dd>
	    ...
	    <div>
	      <dt>Average Visible Salary</dt>
	      <dd>${escapeHtml(formatCurrency(payload.average_visible_salary))}</dd>
	    </div>
    <div>
      <dt>Visible Rows</dt>
      <dd>${escapeHtml(payload.visible_rows)}</dd>
    </div>
  `;
  raw.textContent = JSON.stringify(payload, null, 2);
  refreshAuditEventsQuietly();
...
function renderEmployees(rows) {
  if (!rows.length) {
    employeeRows.innerHTML = '<tr><td colspan="9">No visible rows.</td></tr>';
    return;
  }
  employeeRows.innerHTML = rows.map((row) => `
    <tr>
      <td>${escapeHtml(valueFor(row, "employee_id"))}</td>
      <td>${renderEditableCell(row, "first_name", "can_update_first_name", "text")}</td>
      <td>${escapeHtml(valueFor(row, "last_name"))}</td>
      <td>${escapeHtml(valueFor(row, "job_code"))}</td>
...
function renderAuditEvents(events) {
  if (!events.length) {
    auditRows.innerHTML = '<tr><td colspan="7">No audit records in the last 3 minutes, or audit diagnostics are not configured. Run <code>./setup_audit_diagnostics.sh</code> if needed.</td></tr>';
    return;
  }
