...
async function refreshUser() {
  ...
  if (!user) {
    ...
    tokenDebug.textContent = "Sign in to view authentication mode.";
    contextDebug.textContent = "Sign in to view database context.";
    preflightResults.textContent = "Sign in to run preflight checks.";
    tokenFlow.textContent = "Sign in to view the authentication flow.";
    return null;
  }
...
async function loadTokenDebug() {
  tokenDebug.textContent = "Loading authentication details...";
  try {
...
function renderRequestContext(context) {
  ...
  const databaseSession = context.database_session || context.pooled_connection || {};
  const roles = (context.active_data_roles || []).map((role) => valueFor(role, "role_name")).filter(Boolean);
  requestContext.innerHTML = `
    <div><dt>Database Session</dt><dd>${escapeHtml(databaseSession.session_id)}</dd></div>
    <div><dt>Service</dt><dd>${escapeHtml(databaseSession.service_name)}</dd></div>
...
function renderTokenFlow(payload) {
  const user = payload.user || {};
  const database = payload.database_auth || {};
  tokenFlow.innerHTML = `
    ${flowStep("Local selection", user.username || "Signed-in user", "Browser session cookie")}
    ${flowArrow()}
    ${flowStep("Direct database login", database.database_identity || user.username || "", "Password-based Oracle end user")}
    ${flowArrow()}
