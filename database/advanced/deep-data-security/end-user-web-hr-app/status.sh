...
PID_FILE="${WEB_HR_PID_FILE:-${SCRIPT_DIR}/.end-user-web-hr-app.pid}"
LOG_DIR="${WEB_HR_LOG_DIR:-${SCRIPT_DIR}/logs}"
LOG_FILE="${WEB_HR_LOG_FILE:-${LOG_DIR}/end-user-web-hr-app.log}"
...
if [ ! -f "$PID_FILE" ]; then
  echo "End User Web HR App is not running. No PID file found."
  ...
  if command -v ss >/dev/null 2>&1; then
    ...
    if [ -n "$listener" ]; then
      ...
      echo "$listener"
      echo
      echo "Stop that process, or start End User Web HR App on another port:"
...
if [ -z "$pid" ]; then
  echo "End User Web HR App status is unknown. PID file is empty:"
  echo "  ${PID_FILE}"
  exit 1
fi

if kill -0 "$pid" >/dev/null 2>&1; then
  echo "End User Web HR App is running."
  echo "  PID  = ${pid}"
  echo "  Port = ${WEB_HR_PORT}"
  echo "  Log  = ${LOG_FILE}"
else
  echo "End User Web HR App is not running, but a stale PID file exists."
...
if [ -f "$LOG_FILE" ]; then
  ...
  grep -E "TLS enabled|End User Web HR App running|Database mode" "$LOG_FILE" | tail -n 8 || true
fi
