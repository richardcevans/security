...
If your shell inherited `WALLET_DIR` or `TNS_ADMIN` from another database home, clear them before running `05_configure_network.sh` or the verification scripts:
...
Do not run `05_configure_network.sh` unchanged on production, RAC, Grid Infrastructure, SCAN listener, shared Oracle home, or Data Guard environments.
...
- `03_preflight.sh` has no blocking failures
- `08_verify_db_setup.sh` shows `identity_provider_type=AZURE_AD`
- `08_verify_db_setup.sh` shows `HRAPP_EMPLOYEES` maps to `azure_role=EMPLOYEES`
- `08_verify_db_setup.sh` shows `HRAPP_MANAGERS` maps to `azure_role=MANAGERS`
...
`05_configure_network.sh` rewrites these files in `$ORACLE_HOME/network/admin`:
...
On production, RAC, Grid Infrastructure, SCAN listener, shared Oracle home, or Data Guard environments, do not run `05_configure_network.sh` as-is. Adapt the listener, wallet, and TNS changes manually with your normal DBA change process.

## Network File Backups And Restore

`05_configure_network.sh` creates backup files before rewriting network configuration.
...
Azure CLI is required for `02_setup_entra_id.sh`. The script uses Azure CLI and Microsoft Graph to create and update Entra ID app registrations, enterprise applications, app roles, API permissions, and optional user role assignments.

### Oracle Linux

````
<copy>OS_MAJOR=$(rpm -E %{rhel})
case "$OS_MAJOR" in
  8)
...
````
<copy>./02_setup_entra_id.sh</copy>
````
...
````
<copy>./02_verify_entra_id_setup.sh</copy>
````
...
To use different existing Entra users, set these before running `02_setup_entra_id.sh`:
...
The Azure portal steps below are a fallback if your environment does not allow Azure CLI app administration. If you use `02_setup_entra_id.sh`, skip the manual portal tasks and continue with Part 2.
...
### Script 4: Set database identity provider parameters

Configure the database to accept Entra ID tokens. If you used `02_setup_entra_id.sh`, these values come from `.entra-id-data-grants.env`.
...
````
<copy>./04_configure_db_identity_provider.sh</copy>
````
...
### Script 5: Configure TCPS listener, sqlnet.ora, and tnsnames.ora
...
````
<copy>./05_configure_network.sh</copy>
````
...
If you connect from the VS Code Oracle SQL Developer extension or another Oracle client outside the lab VM, that client must trust the database server certificate created by `05_configure_network.sh`. This is still one-way TLS: the client trusts the database server certificate, and the database does not request a client certificate.

Run this helper on the DBSec-Lab VM after `05_configure_network.sh`:
...
Copy the zip file to the client machine. For Oracle Instant Client systems that do not have `orapki`, copy the generated `hrdb-<hostname>-<pdb>` wallet directory to the path shown in `tnsnames-client-snippet.ora`. For JDBC clients, you can also use the generated `db_server_truststore.p12`. In all cases, keep `SSL_SERVER_DN_MATCH=YES` in the client connect descriptor.

## Part 3: Create Deep Data Security Objects

### Script 6: Create the HR schema and employee data
...
Before running, set your Entra ID tenant domain (in addition to the variables from Scripts 4-5):
...
````
<copy>./06_create_hr_schema.sh</copy>
````
...
### Script 7: Create data roles, data grants, and end user context
...
````
<copy>./07_create_data_roles_and_grants.sh</copy>
````
...
````
<copy>./08_verify_db_setup.sh</copy>
````

### Script 9: Connect and verify as Marvin
...
````
<copy>./09_verify_as_marvin.sh</copy>
````
...
### Script 10: Connect and verify as Emma
...
````
<copy>./10_verify_as_emma.sh</copy>
````
...
````
<copy>./11_cleanup_entra_id.sh</copy>
````

To skip the confirmation prompt:

````
<copy>./11_cleanup_entra_id.sh -f</copy>
````
...
| Script | Purpose |
|---|---|
| `02_setup_entra_id.sh` | Create or reuse Entra app registrations, enterprise apps, roles, permissions, and optional assignments |
| `02_verify_entra_id_setup.sh` | Verify Entra app registrations, enterprise apps, roles, scopes, and permissions |
| `03_preflight.sh` | Check local database, listener tools, and browser-launch readiness |
| `04_configure_db_identity_provider.sh` | Set identity\_provider\_type and identity\_provider\_config |
| `05_configure_network.sh` | Create wallet, configure TCPS listener, sqlnet.ora, tnsnames.ora |
| `06_create_hr_schema.sh` | Create HR schema (NO AUTHENTICATION) with employee data |
| `07_create_data_roles_and_grants.sh` | Create data roles (MAPPED TO azure\_role), data grants, context |
| `08_verify_db_setup.sh` | Verify database-side identity provider, data roles, grants, and HR rows |
| `lib_network_check.sh` | Shared check that `hrdb` resolves before verification scripts connect |
| `09_verify_as_marvin.sh` | Connect as Marvin via Entra ID — 4 rows |
| `10_verify_as_emma.sh` | Connect as Emma via Entra ID — 1 row |
| `11_cleanup.sh` | Drop everything, reset identity provider |
| `11_cleanup_entra_id.sh` | Delete lab-created Entra app registrations and enterprise applications |
...
|---|---|---|
| `03_preflight.sh` | Yes | Read-only checks. |
| `04_configure_db_identity_provider.sh` | Yes | Reapplies identity-provider parameters. |
| `05_configure_network.sh` | Yes in the lab VM | Rewrites network files and restarts listener; use caution outside the lab VM. |
| `06_create_hr_schema.sh` | Yes, destructive for HR | Drops and recreates `HR`, so lab data is reset. |
| `07_create_data_roles_and_grants.sh` | Yes | Recreates data roles, grants, context, and direct logon role. |
| `08_verify_db_setup.sh` | Yes | Read-only verification. |
| `09_verify_as_marvin.sh` | Yes | Requires Marvin Entra login. Browser session cache may reuse another user. |
| `10_verify_as_emma.sh` | Yes | Requires Emma Entra login. Browser session cache may reuse another user. |
| `11_cleanup.sh` | Yes | Removes database-side lab objects and resets identity-provider parameters. |
...
| `ORA-12154` or `Cannot find alias hrdb` | `hrdb` alias is missing from the active `tnsnames.ora` | Rerun `05_configure_network.sh`; check `TNS_ADMIN` |
| `ORA-12514` for `FREEPDB1` | PDB service not registered with listener | Rerun `05_configure_network.sh`; verify `lsnrctl status` shows `freepdb1` or `FREEPDB1` |
| `ORA-01017` after Entra login | Token validation or role/session problem | Check database alert log and rerun `04_configure_db_identity_provider.sh` |
...
| `tnsping hrdb` fails | Network files were not configured or wrong `ORACLE_HOME` | Rerun `05_configure_network.sh`; check `$ORACLE_HOME/network/admin/tnsnames.ora` |
| HR direct login succeeds | HR was not created with `NO AUTHENTICATION` | Rerun `06_create_hr_schema.sh` |
