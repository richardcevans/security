...
If you connect from SQLcl, the VS Code Oracle SQL Developer extension, or another Oracle client outside the lab VM, that client must trust the database server certificate created by `05_configure_network.sh`. This is still one-way TLS: the client trusts the database server certificate, and the database does not request a client certificate.
