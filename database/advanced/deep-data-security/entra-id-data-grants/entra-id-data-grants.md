...
`02_verify_entra_id_setup.sh` also checks the expected demo-user app-role
assignments when Marvin and Emma exist: Marvin needs `EMPLOYEES` and
`MANAGERS`; Emma needs `EMPLOYEES`. Microsoft Entra Conditional Access and MFA
policies are tenant-admin controls and are not changed or evaluated by this lab.
