...
# Parameter   : None
#
# Notes       : Task 5 (continued) - Verify the migration as Emma.
#               Connects as end user emma@domain.com and runs the SAME query.
#               Emma sees 1 row (self only).
#
...
echo
echo -e "${GREEN}============================================================================${NC}"
echo -e "${GREEN}      Verify Migration: Connect as Emma (Employee)                          ${NC}"
echo -e "${GREEN}============================================================================${NC}"
echo
echo -e "${PURPLE}Emma is an end user with only HRAPP_EMPLOYEES_LOCAL.${NC}"
echo -e "${PURPLE}Her Entra ID identity (emma@${DOMAIN_NAME}) matches the user_name column.${NC}"
echo -e "${PURPLE}Same SQL: SELECT * FROM hr.employees${NC}"
echo -e "${PURPLE}Emma sees only her own row.${NC}"
echo
...
echo -e "${YELLOW}Connecting as end user emma@${DOMAIN_NAME}...${NC}"
...
prompt  - Only HRAPP_EMPLOYEES_LOCAL — no manager role.
prompt ========================================================================
...
prompt  - After migration: 1 row (Emma only).
prompt ========================================================================
