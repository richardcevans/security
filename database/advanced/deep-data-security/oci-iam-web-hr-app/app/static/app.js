...
async function refreshConfig() {
  ...
  modeBanner.innerHTML = "<strong>OCI IAM OAuth mode:</strong> each request connects to ADB with the signed-in user's token and database data grants enforce access.";
}
...
async function refreshUser() {
  ...
  if (!user) {
    userBox.innerHTML = "<strong>Not signed in</strong><br />Use OCI IAM to sign in";
    return null;
  }
...
function showAuthenticationContextDemo(user) {
  raw.textContent = JSON.stringify({
    ...
    authentication: "OCI IAM Authorization Code with PKCE; the ID token signature was verified using the identity domain JWKS.",
    database_access: "Direct ADB connection with the signed-in OCI IAM OAuth token; Deep Data Security data grants apply in the database."
  }, null, 2);
}
