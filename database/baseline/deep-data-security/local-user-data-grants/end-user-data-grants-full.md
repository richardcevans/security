# Getting Started with Deep Data Security
...
This lab covers the very basics of end users, data roles, and data grants. Emma and Marvin are colleagues at the same company — Emma is an employee and Marvin is her manager. They both query the same HR table, and the database makes sure each of them only ever sees the data appropriate for their role.
...
Oracle Database end users are a new class of identity — distinct from traditional database schema users. They authenticate directly to the database with a password, but do not own schemas or objects. Deep Data Security ensures that end users and non-human identities (including agents) operate with least privilege and are monitored through centralized auditing.

#### Data Roles

In the Oracle AI Database, you will create data roles that are assigned to Oracle Database end users. When you write `CREATE DATA ROLE HRAPP_MANAGERS` and grant it to an end user, Oracle activates `HRAPP_MANAGERS` automatically when that end user authenticates.
...
End users land in a null schema — a schema with no privileges. They cannot hold system privileges like `CREATE SESSION` or object privileges like `EXECUTE` directly. So how do they connect to the database or invoke packages?
...
When Emma authenticates and `HRAPP_EMPLOYEES` activates, `DIRECT_LOGON_ROLE` activates with it. Emma's session gains `CREATE SESSION` without Emma ever being granted it directly.

#### Data Grants

Data grants define which DML and SELECT operations are possible by the data role. The predicate uses `ora_end_user_context.username` to identify the user by their *end user* identity and restrict the rows returned to only rows that match their identity.
...
- You have access to a database user with sufficient privileges to create users and roles
- SQL*Plus is installed and accessible
- The database name is `freepdb1`
...
To enforce a least-privilege model, you can create a dedicated `DEEPSEC_ADMIN` user with only the privileges required to complete this lab. Run the following as a database user with the appropriate privileges (typically `SYSTEM` or `SYS`).
...
2. Create the DEEPSEC_ADMIN user with the required privileges:
...
3. For the remaining tasks, connect as `deepsec_admin`:

    ```
    sqlplus deepsec_admin/Oracle123@freepdb1
    ```

    Alternatively, you can continue as SYSTEM if you prefer not to create a separate administrator account.
...
1. Create a schema-only account for the HR data:
...
2. Create the `EMPLOYEES` table and insert sample data:
...
4. Create the `MANAGERS` lookup table that maps employees to their managers:
...
5. Verify all data is visible (before data grants are applied):
    ...
    You should see all 7 employees with all columns visible, including SSNs and salaries. This demonstrates the problem: without data grants, any user with table access sees everything.
